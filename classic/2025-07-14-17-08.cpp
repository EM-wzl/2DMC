...
using namespace std;
int s[1001][1001];
int blockwj[101]={0,50,0,100,200,30,0,0,0,0,-1,0,0};
int beibao[101][2];
int Clocknum=0,jump=0,dx,dy,my,tall,ks=0,mx,fz,cho=1,key_=0,life=20,bx,by,fell=0,co=0,H=0,anx=0;;
...
void wajue(int sx,int sy)
{
	...
	if(beibao[cho][0]==8)wj+=12;	
	if(beibao[cho][0]==7)wj+=8;
	else wj+=4;}
	else return;
	if(s[sx][sy]==2){
...
void block_(int xxx,int yyy)
{
	...
	if(beibao[cho][1]>0)s[xxx][yyy]=beibao[cho][0],beibao[cho][1]--;
}
void Start()
{
	life=20,x=1,y=50;
...
int main()
{
	...
	if (KEY_DOWN(VK_LBUTTON)) {
    		POINT p=GetMousePos();
    		int mpx=To_int(jx-26+p.x/2);
    		int mpy=To_int(jy-14+p.y);
    		if(abs(mpx-int(x+0.5))<=2 && abs(mpy-int(y+0.5))<=2){
    			wajue(mpx,mpy);
  			}
  			else {
	...
	if(spx>0)while(blockwj[s[int(x+0.5)][int(y+0.5)]]>0)x-=0.05;
	if(spx<0)while(blockwj[s[int(x+0.5)][int(y+0.5)]]>0)x+=0.05;
	spx=0;
	}
	jx=x;
